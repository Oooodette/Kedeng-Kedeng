from code.classes import Trajectory, Network
import pandas as pd
import random

def create_available_stations(current_station, connection_list): 
...
def pick_random_station(station_list):
    """"
...
def create_trajectory(station_list, connection_list):
    
    # initialize variables
    current_station = pick_random_station(station_list)
    ...
    while time < 120:
        ...
        available_stations = create_available_stations(current_station, connection_list)
        new_connection = pick_valid_connection(available_stations, previous_connection, time) 

        # if a valid connection is found, change the current station to the next station of this connection
        if new_connection != None:
            current_station = determine_station(current_station, new_connection)
...
def create_network(connections_file, stations_file): 
    ...
    station_list = new_network.stations
    connection_list = new_network.connections
    counter = 1
    # check if all connections are used and keep making trajectories 
    while not new_network.is_valid():
        
        new_trajectory = create_trajectory(station_list, connection_list)
        new_network.add_trajectory(new_trajectory)
        ...
        for connection, value in new_trajectory.connections:
            new_network.used[connection] = value
